document.addEventListener('DOMContentLoaded', function () {
    ...
    window.onload = () => {
        const nomePlayer = localStorage.getItem('nomePlayer');
        if (nomePlayer) {
            const bola = document.createElement('div');
            bola.className = 'bola-vermelha';
            bola.textContent = nomePlayer;
            ...
            document.addEventListener('keydown', (e) => {
                const distancia = 10;
                if (e.key === 'ArrowUp') posY -= distancia;
                if (e.key === 'ArrowDown') posY += distancia;
                if (e.key === 'ArrowLeft') posX -= distancia;
                if (e.key === 'ArrowRight') posX += distancia;
